...
object Exercise {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val res1 = arr.reduce(_ + _) // 这种情况下和 arr.reduceLeft(_+_) 一样 这是从左往右  如果是right的话是从右往左进行


    // 调用了几个线程也是随机的
    val resPar = arr.par.reduce(_ - _) // 你会发现 结果是会变的 这是因为线程数是改变的
    val res1_left = arr.par.reduceLeft(_ - _) // 你会发现 结果是不会变的 因为顺序是固定的
    val res1_right = arr.par.reduceRight(_ - _) // 你会发现 结果是不会变的 因为顺序是固定的
    // TODO 折叠：有初始值(无特定顺序)
    ...
    val res3 = arr.foldLeft(0)(_ + _) // 从左往右开始 就是说第一个下划线_ 是给的初始值0 然后再从arr里拿第一个作为第二个_ 然后才是开始递归
    val res4 = arr.foldRight(0)(_ + _) // 从右往左开始 就是说第一个下划线_ 是给的初始值0 然后再从arr里拿第一个作为第二个_ 然后才是开始递归
    val res3_left = arr.par.foldLeft(0)(_ - _) // 你会发现 结果是不会变的 因为顺序是固定的
    val res4_right = arr.par.foldRight(0)(_ - _) // 你会发现 结果是不会变的 因为顺序是固定的
    ...
    val l2 = List(1, 4, 3)
    // TODO 求并集
    val res9 = l1 union l2 // l1 中的元素 和l2 中2个加起来的元素  可以有重复值

    // TODO 求交集
    val res10 = l1 intersect l2 // l1 中的元素 和 l2中 相同的元素 例如 1

    // TODO 求差集
    val res11 = l1 diff l2 // l1 中的元素 去除 l2 的元素 5,6,7  去除了1
...
object Es {
  ...
  def main(args: Array[String]): Unit = {
    ...
    List("4 5 8 4 1","5 7 8 9 8 7 8 8 5 49 8 7 89 45 54 654","")
    // TODO 先按空格切分 在压平
