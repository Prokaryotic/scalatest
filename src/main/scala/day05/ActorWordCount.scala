...
object ActorWordCount {
  def main(args: Array[String]): Unit = {
    ...
    // 文件路径
    val files = Array("F:\\test\\scala\\words\\a.txt")
    ...
    for (file <- files) {
      val task = new Task
      task.start
      val replyMsg: actors.Future[Any] = task !! SmTask(file)
    ...
    while (true) {
      receive {
        case SmTask(file) => {
          ...
          val res: Map[String, Int] = lines.flatMap(_.split(" ")).map(_ -> 1).groupBy(_._1).mapValues(_.size)
          // 异步发送
          sender ! res
        }
      }
